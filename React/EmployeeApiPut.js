...
class EmployeeApiPut extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getEmployee=()=>{
        axios.get("http://localhost:4001/Emp").then(response=>{
    ...
    updateEmployee = (e) =>{
        ...
        // setTimeout(() => this.getEmployee(), 1000);
        Employee.id=''
    ...
    selectName =(e)=>{
        ...
        let emp = EmpList.find(val=> val.id == value)
    ...
    render() {
        ...
        <div className="row">
            ...
            <div className="col-md-3 m-3">
                <select className="form-select mb-3" onChange={this.selectName}>
                    <option value='0'> Select Employee Name </option>
                    { EmpList.map(item=>{ 
                        return <option key={item.id} value={item.id}> {item.employeeName} </option>    
                        })
                    }
                </select>
            </div>
